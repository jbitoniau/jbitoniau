...
var UIColorPicker = React.createClass({
	displayName: 'UIColorPicker',

	getInitialState: function getInitialState() {
		return { colorjoe: null };
	},

	componentDidMount: function componentDidMount() {
		var containerDiv = React.findDOMNode(this.refs.containerDiv);
		if (!containerDiv) return;

		var extras = ['alpha', 'currentColor', ['fields', { space: 'HSLA', limit: 100 }], ['fields', { space: 'CMYKA', limit: 100 }], 'hex'];
		var cj = colorjoe.rgb(containerDiv, '#0000FF', extras);

		/*cj.on("change", function(color) 
  	{
  		console.log("Selecting " + color.css());
  	});
  		cj.on("done", function(color) {
	...
	render: function render() {
		...
		return(
			...
			React.createElement(
				'div',
				{ style: mainDivStyle },
				React.createElement('div', { style: { position: 'absolute', width: 'auto', height: 'auto' },
					ref: 'containerDiv' })
			)
		);
	}
