...
var UISlider = React.createClass({
	...
	// Used to send notifications only when value changes
	render: function render() {
		...
		mainDivStyle = Object.assign(mainDivStyle, { position: 'relative',
			display: 'flex',
		...
		return(
			...
			React.createElement(
				...
				React.createElement(
					'div',
					{ style: { flex: 'none', height: barThickness } },
					React.createElement(
						'div',
 ...
 */
	...
	_onKnobMouseMove: function _onKnobMouseMove(e) {
	...
	_onKnobMouseUp: function _onKnobMouseUp(e) {
 ...
 */
	...
	_getTouchFromIdentifier: function _getTouchFromIdentifier(touchList, identifier) {
 ...
 */
	_knobDragStart: function _knobDragStart(x, y, eventIdentifier) {
		if (x && y) {
	...
	_knobDragMove: function _knobDragMove(x, y) {
		...
		if (knobPosOnBarX < 0) knobPosOnBarX = 0;else if (knobPosOnBarX > sliderBarWidth) knobPosOnBarX = sliderBarWidth;
	...
	_knobDragEnd: function _knobDragEnd() {
 ...
 */
	_getElement: function _getElement(elementRefName) {
	...
	_convertDocumentPositionToElementPosition: function _convertDocumentPositionToElementPosition(x, y, element) {
	...
	_getKnobNormalizedPositionFromSliderValue: function _getKnobNormalizedPositionFromSliderValue(sliderValue) {
	...
	_getSliderValueFromKnobNormalizedPosition: function _getSliderValueFromKnobNormalizedPosition(knobNormalizedPosition) {
		var min = this.props.min;
		var max = this.props.max;
...
/* The rectangle with fixed height for the bar and the knob */ /* The bar */ /* The knob anchor point placed at proper position on the bar */ /* The knob image (correctly centered on parent anchor) */